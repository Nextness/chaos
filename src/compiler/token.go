...
//   - TkIdent, TkDirec: NFC-normalized identifier text
...
// Text returns the semantic token text. Identifiers and directive names are
// normalized to NFC; punctuation and literals retain their raw spelling.
func (t Token) Text() string {
	if t.Kind == TkIdent || t.Kind == TkDirec {
		return t.Value
	}
	return string(t.Raw)
}
